...
fn test_extract_pdf_minimal_extracts_pages() {
    ...
    assert!(!doc.pages.is_empty(), "should have at least one page");
}
...
fn test_extract_pdf_minimal_contains_text() {
    ...
    assert!(
        all_text.contains("Lorem"),
        "should find body text: {}",
        all_text
    );
}
...
fn test_extract_pdf_detects_fonts() {
    ...
    let doc = extract_pdf(bytes).expect("extraction should succeed");
    assert!(
        !doc.metadata.detected_fonts.is_empty(),
        "should detect at least one font"
    );
}
